...
lbl_page_info = tk.Label(window)
lbl_page_info.pack(side=tk.BOTTOM, pady=5)
...
btn_xoa_du_lieu.pack(pady=10)  # Đặt nút ở giữa khung
...
def go_to_page():
    ...
    try:
        ...
        if 1 <= page <= total_pages:
            current_page = page - 1  # Đặt current_page vào chỉ số tương ứng
            hien_thi_du_lieu()
        else:
...
def sap_xep_giam():
    ...
    try:
        ...
        data['ID'] = data['ID'].astype(str)  # chuyển ngược lại str
        data.reset_index(drop=True, inplace=True)  # Đặt lại chỉ số sau khi sắp xếp
        hien_thi_du_lieu()  # Hiển thị dữ liệu đã sắp xếp
    ...
    try:
        ...
        if filtered_data.empty:
            ...
            for _, row in filtered_data.iterrows():
                tree.insert("", "end", values=list(row))
            current_page = 0  # Đặt lại trang về 0 khi lọc
            update_page_info()
...
frame_sap_xep_du_lieu_theo_id.pack_forget()
frame_trich_loc.pack_forget()
frame_xoa_du_lieu.pack_forget()  # Đảm bảo khung Xóa dữ liệu được ẩn lúc khởi chạy
